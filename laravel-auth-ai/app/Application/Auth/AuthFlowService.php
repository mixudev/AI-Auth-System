...
use App\Services\Auth\Mfa\MfaManager;
...
class AuthFlowService
{
    ...
    public function attemptLogin(Request $request, bool $remember = false): array
    {
        ...
        try {
            ...
            try {
                ...
                // Jangan biarkan kegagalan notifikasi menyembunyikan error asli
    ...
    public function verifyMfa(Request $request, string $sessionToken, string $code): array
    {
        ...
        // Baik EmailMfaStrategy maupun TotpMfaStrategy menyimpan getDeviceSignature()
        // ke kolom device_fingerprint saat generate(). Kita bandingkan dengan nilai
        // yang sama saat verifikasi. Guard null jika OTP lama tidak memiliki signature.
    ...
    private function loginWithWhitelistedIp(Request $request, bool $remember): array
    {
        ...
        // User block dan MFA tetap wajib diperiksa — bahkan dari IP terpercaya.
        if ($this->blockingService->isUserBlocked($user->id)) {
    ...
    private function completeAuthenticatedSession(
        ...
        // [M-02 FIX] Untuk perangkat baru (belum trusted), rotate cookie UUID
        // agar cookie yang mungkin dicuri sebelum login tidak bisa dipakai bypass trusted device.
        ...
        if ($isNewDevice && $request->hasSession()) {
            // Generate device ID baru dan set sebagai cookie di response
            ...
            \Illuminate\Support\Facades\Cookie::queue(
                \App\Http\Middleware\DeviceIdentifierMiddleware::COOKIE_NAME,
                $newDeviceId,
                $cookieMinutes,
                '/',
                null,
