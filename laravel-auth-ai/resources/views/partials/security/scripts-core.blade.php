...
const PAGE_META = {
  ...
  logs:      { title: 'Login Logs',         sub: 'All authentication attempts',       cols: 11 },
  devices:   { title: 'Trusted Devices',    sub: 'Registered & revoked devices',      cols: 10 },
  users:     { title: 'Users',              sub: 'User accounts & block management',  cols: 14 },
  ...
  errors:    { title: 'System Errors',      sub: 'Backend & security exception logs', cols: 8  },
};
...
function renderRow(tab, r, n) {
  switch (tab) {
    ...
    case 'errors':    return rowSystemError(r, n);
    default:          return '';
  }
}

function rowSystemError(e, n) {
  const tc = { error: 'b-red', warning: 'b-amber', info: 'b-cyan' }[e.type] || 'b-muted';
  return `<tr>
    <td class="dim">${n}</td>
    <td class="mono-sm">${e.occurred_at}</td>
    <td><span class="badge ${tc}">${e.type.toUpperCase()}</span></td>
    <td class="mono-sm">${esc(e.event)}</td>
    <td><strong>${esc(e.title)}</strong></td>
    <td class="dim trunc" style="max-width:300px">${esc(e.message)}</td>
    <td class="mono-sm">${esc(e.ip_address || '—')}</td>
    <td>
      <button class="btn btn-sm btn-ghost" onclick="viewErrorDetails(${e.id})">🔍 Details</button>
    </td>
  </tr>`;
}

async function viewErrorDetails(id) {
  try {
    // Cari data di local state dulu (opsional, tapi lebih cepat)
    // Untuk simplisitas, fetch ulang agar fresh
    const json = await apiFetch('GET', '/system-errors');
    const logs = json.data ?? [];
    const log  = logs.find(l => l.id === id);
    
    if (!log) { toast('Error data not found', 'err'); return; }

    const meta = log.meta || {};
    const html = `
      <div class="error-modal-content">
        <div class="error-meta-item">
          <span class="error-meta-label">Title</span>
          <div style="font-weight:600;color:white">${esc(log.title)}</div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">Message</span>
          <div>${esc(log.message)}</div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">Source</span>
          <div class="mono-sm">${esc(meta.file || 'Unknown')}:${meta.line || '?'}</div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">Exception</span>
          <div class="badge b-red">${esc(meta.exception || 'N/A')}</div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">Stack Trace</span>
          <div class="error-trace">${esc(meta.trace || 'No trace available')}</div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">User Context</span>
          <div class="dim">IP: ${esc(log.ip_address)} | Email: ${esc(meta.email || 'N/A')}</div>
          <div class="dim" style="font-size:10px;margin-top:4px">${esc(log.user_agent)}</div>
        </div>
      </div>
    `;

    // Overload modal utility
    document.getElementById('m-title').textContent = 'System Error Details';
    document.getElementById('m-body').innerHTML    = html;
    document.getElementById('m-inp').style.display = 'none';
    document.getElementById('m-ok').textContent    = 'Close';
...
function rowLog(l, n) {
  ...
  return `<tr class="${l.decision === 'BLOCK' ? 'row-blocked' : ''}">
    ...
    <td>
      <button class="btn btn-sm btn-ghost" onclick="viewLogDetails(${l.id})">🔍 View</button>
    </td>
  </tr>`;
}
...
async function viewLogDetails(id) {
  try {
    // Cari data di local (karena kita fetch log detail di controller sekarang)
    // Tapi untuk memastikan data terbaru/lengkap, kita bisa fetch ulang jika perlu.
    // Di sini kita asumsikan data sudah ada di 'logs' tab cache/state jika user baru saja load.
    // Namun API DevLoginLogController mengembalikan data yang cukup lengkap.
    const resp = await fetch(BASE + '/logs', {
      headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': CSRF },
    });
    const json = await resp.json();
    const log  = (json.data ?? []).find(l => l.id === id);

    if (!log) { toast('Log data tidak ditemukan', 'err'); return; }

    const aiRaw = log.ai_raw || {};
    const html = `
      <div class="error-modal-content">
        <div class="error-meta-item">
          <span class="error-meta-label">Audit Info</span>
          <div style="font-weight:600;color:white">Log ID: #${log.id} — ${log.occurred_at}</div>
        </div>
        <div class="error-meta-item" style="display:flex;gap:20px">
          <div style="flex:1">
            <span class="error-meta-label">IP Address</span>
            <div class="mono-sm">${esc(log.ip_address)}</div>
          </div>
          <div style="flex:1">
            <span class="error-meta-label">Country</span>
            <div class="badge b-cyan">${esc(log.country_code || '??')}</div>
          </div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">User Agent</span>
          <div class="dim" style="font-size:11px">${esc(log.user_agent)}</div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">Decision Strategy</span>
          <div style="margin-bottom:8px">
            <span class="badge ${log.decision === 'ALLOW' ? 'b-green' : log.decision === 'BLOCK' ? 'b-red' : 'b-amber'}">${log.decision}</span>
            <span class="badge b-muted">Score: ${log.risk_score}</span>
          </div>
          <div class="flags">${(log.reason_flags || []).map(f => `<span class="flag">${esc(f)}</span>`).join('')}</div>
        </div>
        <div class="error-meta-item">
          <span class="error-meta-label">AI Response Raw (JSON)</span>
          <div class="error-trace" style="color:#94a3b8;font-size:11px">${JSON.stringify(aiRaw, null, 2)}</div>
        </div>
      </div>
    `;

    document.getElementById('m-title').textContent = 'Audit Detail: ' + (log.user || log.email);
    document.getElementById('m-body').innerHTML    = html;
    document.getElementById('m-inp').style.display = 'none';
    document.getElementById('m-ok').textContent    = 'Close';
    document.getElementById('overlay').classList.add('open');
...
function rowUser(u, n) {
  ...
  return `<tr class="${blocked ? 'row-blocked' : ''}">
    ...
    <td class="mono-sm">${esc(u.last_login_at)}</td>
    <td class="mono-sm">${esc(u.last_login_ip)}</td>
    <td><span class="badge b-cyan">${esc(u.last_login_country)}</span></td>
    <td class="dim" style="font-size:10px">${esc(u.last_login_device)}</td>
    <td class="mono-sm">${u.login_count}</td>
    <td class="mono-sm">${u.device_count}</td>
