...
_RULE_WEIGHTS = {
    ...
    "trust_bonus":           -50,  # Bonus kepercayaan untuk perangkat yang sudah terverifikasi
}
...
def evaluate(data: RiskInputSchema) -> RuleBasedResult:
    ...
    # -- Aturan 9: Trust Override (Bonus Kepercayaan Perangkat)
    # Jika perangkat sudah terdaftar dan masih aktif, beri diskon besar pada skor risiko.
    # Threshold diturunkan ke 0.7 agar device yang baru saja diverifikasi (skor
    # proporsional, bukan binary 1.0) tetap mendapat bonus ini sebagai failsafe.
    if data.device_trust_score >= 0.7:
        raw_score += _RULE_WEIGHTS["trust_bonus"]
        reason_flags.append("trusted_device_bypass")
